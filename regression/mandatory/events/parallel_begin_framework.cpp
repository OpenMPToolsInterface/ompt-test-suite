 ...
 )
{
  CHECK(parent_task_id != 0,		\
	IMPLEMENTED_BUT_INCORRECT, \
        ...
        "parallel_id = %lu", parallel_id);

  CHECK(parallel_id != 0,		\
        IMPLEMENTED_BUT_INCORRECT,			  \
        "parallel region id = 0 in event_parallel_begin " \
        "parent_task_id = %lu", parent_task_id);
...
#if DEBUG
  printf("begin: parallel region id = %lu, parent_task_id = %lu\n", 
  ...
  CHECK(iter == parallel_id_set.end(), \
        IMPLEMENTED_BUT_INCORRECT, \
        "duplicate parallel region id %lu in region set", *iter);
...
fib_region_nesting(int n, int depth)
{
  ...
  #pragma omp parallel num_threads(NUM_THREADS)
  {
    ...
    {
      pthread_mutex_lock(&thread_mutex);
      printf("%*s enter region id %lu, task id = %lu, "
	     "parent task id %lu (threads = %d)\n", 
    ...
    CHECK(parent_task_id == parallel_id_to_task_id_map[parallel_id],	\
	  IMPLEMENTED_BUT_INCORRECT,					\
	  "ompt_get_task_id(1) = %lu does not match task %lu that created " \
	  "region ompt_get_parallel_id(0)=%lu", parent_task_id, \
    ...
    CHECK(parent_frame == parallel_id_to_task_frame_map[parallel_id],	\
	  ...
	  "region ompt_get_parallel_id(0)=%lu", parent_frame, parallel_id);
    pthread_mutex_unlock(&thread_mutex);
    ...
    {
      pthread_mutex_lock(&thread_mutex);
      printf("%*s exit region id %lu, task id = %lu, parent task id %lu\n", 
    ...
    #pragma omp parallel num_threads(NUM_THREADS)
    {
      ...
      CHECK(ompt_get_task_id(1) == parallel_id_to_task_id_map[level2_parallel_id], \
	    ...
	    "level 2 parent task id does not match: " \
            "expected parent %lu, ompt_get_task_id(1) = %lu", \
