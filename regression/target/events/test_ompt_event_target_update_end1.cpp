...
void update_end_test() {
    ...
    CHECK(number_begin_events == 1, IMPLEMENTED_BUT_INCORRECT, "copy variable from host to device: number of target_task_begin events with task_type = ompt_target_task_update not as expected (expected %d, observed %d)", 1, number_begin_events);
