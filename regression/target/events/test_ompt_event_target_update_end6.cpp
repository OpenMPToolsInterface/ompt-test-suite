...
void update_end_test() {
    ...
    CHECK(number_begin_events == 0, IMPLEMENTED_BUT_INCORRECT, "no update, only target data region: number of target_task_begin events with task_type = ompt_target_task_update not as expected (expected %d, observed %d)", 0, number_begin_events);
